...
if (strpos($r1, '"rate_limit"') !== false ||  strpos($r1, '"processing_error"') !== false) {
    $loop_count++;
    $rate_limit_reached = false; 
}
elseif (strpos($r1, '"card_not_supported"') || strpos($r1, '"invalid_expiry_year"') || strpos($r1, '"invalid_cvc"') || strpos($r1, '"invalid_expiry_month"') || strpos($r1, '"incorrect_number"') || strpos($r1, '"generic_decline"') || strpos($r1, '"parameter_invalid_integer"')) {
    ...
    echo '<span style="color:red">========================================<br>RESULT: #DEAD ❌ <br> ➔CARD:- `'.$lista.'` <br> ➔MASSAGE: '.$resp1.' ┃ '.$msg1.'<br> ➔BIN DATA: '.$ci.'<br> ➔BYPASSING: '.$loop_count.'</span><br>';
...
elseif (strpos($r1, '"testmode_charges_only"') || strpos($r1, '"api_key_expired"')){
  $rate_limit_reached = true;
...
$ci1 = " $type1 ┃ $country1 ";
//////////================[DEAD]==============///
if (strpos($r2, '"rate_limit"') !== false || strpos($r2, '"processing_error"') !== false || (strpos($r2, '"try_again_later"') !== false || strpos($r2, '"parameter_invalid_integer"') !== false)) {
    $loop_count++;
  $rate_limit_reached = false;
} 
...
elseif (strpos($r2, '"invalid_account"') || strpos($r2, '"expired_card"') || strpos($r2, '"fraudulent"') !== false || strpos($r2, '"pickup_card"') || strpos($r2, '"incorrect_number"') || strpos($r2, '"lost_card"') || strpos($r2, '"generic_decline"') || strpos($r2, '"stolen_card"') || strpos($r2, '"card_not_supported"') || strpos($r2, '"card_decline_rate_limit_exceeded"')) {
    $rate_limit_reached = true;
    echo '<span style="color:red">========================================<br>RESULT: #DEAD ❌ <br> ➔CARD:- `'.$lista.'` <br> ➔MASSAGE: '.$resp2.' ┃ '.$msg2.'<br> ➔BIN DATA: '.$ci.'<br> ➔BYPASSING: '.$loop_count.'</span><br>';
...
elseif (strpos($r2,'"do_not_honor"') || strpos($r2,'"incorrect_cvc"') || strpos($r2, '"invalid_cvc"')){
      $rate_limit_reached = true;
    $sendccn = true;
    echo '<span style="color:Aqua">========================================<br>RESULT: #CCN ❎ <br> ➔CARD:- `'.$lista.'` <br> ➔MASSAGE: '.$resp2.' ┃ '.$msg2.'<br> ➔BIN DATA: '.$ci.'<br> ➔BYPASSING: '.$loop_count.'</span><br> ';
if (strpos($r2, '"risk_level":')) {
  echo '<span style="color:red">➔risk_level:'.$riskl.'</span><br>';
}
  if (strpos($r2, '"risk_score"')) {
...
elseif (strpos($r2,'"insufficient_funds"') || strpos($r2,'"authentication_required"') || strpos($r2,'"transaction_not_allowed"')){
  $rate_limit_reached = true;
    $sendcvv = true;
    echo '<span style="color:#66ff66">========================================<br>RESULT: #CVV ✔️ <br> ➔CARD:- `'.$lista.'`<br> ➔MASSAGE: '.$resp2.' ┃ '.$msg2.'<br> ➔BIN DATA: '.$ci.'<br> ➔BYPASSING: '.$loop_count.'</span><br> ';
if (strpos($r2, '"risk_level":')) {
  echo '<span style="color:red">➔risk_level:'.$riskl.'</span><br>';
}
if (strpos($r2, '"risk_score"')) {
...
elseif (strpos($r2,'"status": "succeeded"') !== false || strpos($r2,'"seller_message": "Payment complete."') !== false || strpos($r2,'"approved_by_network"') !== false){
    $rate_limit_reached = true;
    $sendlive = true;
    echo '<span style="color:Aqua">========================================<br>RESULT: #LIVE CHARGED $'.$crn.' ✅ <br> ➔CARD:- `'.$lista.'`<br> ➔MASSAGE: '.$resp2.' ┃ '.$msg2.'<br>➔cvc_check:'.$cvc.'<br>➔Receipt :<a href='.$receipturl.'>Here</a><br> ➔BIN DATA: '.$ci.'<br> ➔BYPASSING: '.$loop_count.'</span><br> ';
if (strpos($r2, '"risk_level":')) {
  echo '<span style="color:red">➔risk_level:'.$riskl.'</span><br>';
}
  if (strpos($r2, '"risk_score"')) {

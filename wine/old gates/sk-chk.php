...
$code = trim(strip_tags(getStr($result1,'"code": "','"')));
...
if (strpos($result1, "testmode_charges_only")) {
    echo '<span style="color:Aqua">CCN:'.$sk.'<br>['.$code.']====[Result(R1): '.$msg1.']</span><br>';
}
elseif(strpos($result1, "api_key_expired" )) {
    echo '<span style="color:Aqua">CCN:'.$sk.'<br>['.$code.']====[Result(R1): '.$msg1.']</span><br>';
}
elseif(strpos($result1, "Invalid API Key provided:" )) {
    echo '<span style="color:red">DEAD:'.$sk.'<br>['.$code.']====[Result(R1): '.$msg1.']</span><br>';
}
elseif(strpos($result2, "Invalid API Key provided:" )) {
    echo '<span style="color:red">DEAD:'.$sk.'<br>['.$code.']====[Result(R1): '.$msg1.']</span><br>';
}
else {
    echo '<span style="color:#33cc33">LIVE:'.$sk.'<br>[Result(R1): '.$msg1.']===['.$code.']</span> <br>[Result(R2): '.$msg1.']====['.$code.']</span><br>';    
}
  curl_close($ch);
  ob_flush();
?>